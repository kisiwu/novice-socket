import { Middleware, SocketMiddleware, ListenerBuilderJson, Controller, ListenerBuilderSettings } from './definitions';
...
export class NspBuilder {
    ...
    constructor(name = '/') {
        ...
        if (name) {
            if (typeof name !== 'string') {
                throw new TypeError("'NspBuilder' first argument should be a string");
            }
            if (!name.startsWith('/')) {
                throw new SyntaxError("'NspBuilder' first argument should be a string starting with '/'");
            }
            this.#name = name;
        }
    }
     ...
     */
    add<DataType, ErrorType>(
        name: string | ListenerBuilderSettings,
    ...
    add(
        ...
        if (
            ...
            if (typeof name === 'string' && args.some((arg) => arg instanceof NspBuilder)) {
                return this.link(name, ...args.filter((arg) => arg instanceof NspBuilder));
            } else {
                const handlers: Controller[] = args.filter((arg) => typeof arg === 'function') as Controller[];
        ...
        if (!treatedFirstArg) {
            ...
            firstArg.forEach((arg) => {
                if (arg instanceof NspBuilder) {
                    ...
                    socketMiddlewares.push(arg as SocketMiddleware);
                }
            });
        }
        ...
        if (listenerBuilders.length) {
            ...
            newEvents = newEvents.map((ev) => ev).reverse();
            newEvents = newEvents
                .filter((a, idx) => {
                    return newEvents.findIndex((b) => b.name === a.name) == idx;
                })
                .sort((a, b) => {
                    let sort = 0;
                    if (a.name > b.name) {
    ...
    registerMiddlewares(namespace: string, nsp: Namespace, io: Server): void {
        ...
        if (this.#stack.length && namespace.startsWith(this.name)) {
            ...
            this.#stack.forEach((router) => router.registerMiddlewares(subpath, nsp, io));
        }
    }

    registerEvents(namespace: string, socket: Socket, nsp: Namespace, io: Server): void {
        if (namespace == this.name) {
            this.#socketMiddlewares.forEach((middleware) => {
        ...
        if (this.#stack.length && namespace.startsWith(this.name)) {
            ...
            this.#stack.forEach((router) => router.registerEvents(subpath, socket, nsp, io));
        }
    }
    ...
    getPaths(namespace?: string): string[] {
        ...
        this.#stack.forEach((router) => {
            flatten(router.getPaths(namespace)).forEach((p) => {
                if (v.indexOf(p) == -1) {
                    v.push(p);
                }
            });
