...
export class ServerApp {
    ...
    build(srv?: number | Partial<ServerOptions> | HttpServer | undefined, opts?: Partial<ServerOptions>): ServerApp {
        ...
        this.getPaths().forEach((namespace) => {
            const nsp = server.of(namespace);

            // register nsp middlewares
            mainRouter.registerMiddlewares(namespace, nsp, server);

            // connection event
            nsp.on('connection', (socket) => {
                // socket.handshake.auth;

                this.#onConnectionFn.forEach((fn) => fn(socket, nsp, server));

                // disconnect event
                socket.on('disconnect', (reason) => {
                    this.#onDisconnectFn.forEach((fn) => fn(reason, socket, nsp, server));
                });

                // register events
                mainRouter.registerEvents(namespace, socket, nsp, server);
            });
        });
    ...
    onDisconnect(
        ...
        const args = toArray<(reason: string, socket: Socket, nsp: Namespace, server: Server) => void>(
            flatten(fn),
            'function'
        );
